...
for (h in seq_along(dates_test)) {
  ...
  DELTA_PLAFOND <- 0.8  # force au moins 20% de poids aux indicateurs, meme
                         # pour les branches ou l'optimisation en echantillon
                         # avait mis tout le poids sur le BVAR (Peche, Construction)
  deltas <- pmin(deltas, DELTA_PLAFOND)
...
saveRDS(list(dates_test = dates_test, resultats_etages = resultats_etages,
        ...
        file.path(DOSSIER_RESULTATS, "test_affinement_delta_plafonne.rds"))
...
ggsave(file.path(DOSSIER_FIGURES, "09b_affinement_delta_plafonne.png"), p, width = 8, height = 5.5, dpi = 150)
