...
# --- Indicatrice Covid (T2-2020, T3-2020) ----------------------------------
# Les deux trimestres les plus perturbes par le choc Covid (ex. -0,86 pour
# Hebergement-restauration, -0,50 pour Transports au T2-2020) -- ajoutee
# comme regresseur exogene au BVAR, avec un prior aussi lache que celui de
# la constante (ne contraint pas sa valeur a priori).
...
construire_XY <- function(Y, p, covid) {
  Tn <- nrow(Y); n <- ncol(Y)
  Y_reg <- Y[(p+1):Tn, , drop = FALSE]
  X_reg <- matrix(1, nrow = Tn - p, ncol = 2 + n * p)  # col 1 = constante, col 2 = covid
  X_reg[, 2] <- covid[(p+1):Tn]
  for (l in 1:p) {
    X_reg[, (3 + (l-1)*n):(2 + l*n)] <- Y[(p+1-l):(Tn-l), , drop = FALSE]
  }
...
# Bloc 3 : prior lache sur la constante ET sur l'indicatrice Covid (grande
#          variance = peu informatif pour les deux)
construire_dummies <- function(sigma_i, n, p, lambda) {
  Yd1 <- matrix(0, nrow = n * p, ncol = n)  # delta_i = 0 => cible nulle sur le 1er retard
  Xd1 <- matrix(0, nrow = n * p, ncol = 2 + n * p)
  for (l in 1:p) {
    rows <- ((l-1)*n + 1):(l*n)
    Xd1[rows, (3 + (l-1)*n):(2 + l*n)] <- diag(sigma_i * l / lambda)
  }
  Yd2 <- matrix(0, nrow = n, ncol = n); diag(Yd2) <- sigma_i
  Xd2 <- matrix(0, nrow = n, ncol = 2 + n * p)

  eps <- 1e-5
  Yd3 <- matrix(0, nrow = 2, ncol = n)
  Xd3 <- matrix(0, nrow = 2, ncol = 2 + n * p)
  Xd3[1, 1] <- eps  # constante
  Xd3[2, 2] <- eps  # covid

  list(Y = rbind(Yd1, Yd2, Yd3), X = rbind(Xd1, Xd2, Xd3))
}

estimer_bvar <- function(Y, p, lambda, sigma_i, covid) {
  n <- ncol(Y)
  reel <- construire_XY(Y, p, covid)
  dum <- construire_dummies(sigma_i, n, p, lambda)
  Y_aug <- rbind(reel$Y, dum$Y)
  X_aug <- rbind(reel$X, dum$X)
  B <- solve(t(X_aug) %*% X_aug) %*% t(X_aug) %*% Y_aug  # (2+np) x n
  residus <- reel$Y - reel$X %*% B
  list(B = B, residus = residus, X_dernier = reel$X[nrow(reel$X), ])
}

modele <- estimer_bvar(Y, P_RETARDS, LAMBDA, sigma_i, covid_dummy)

# --- Prevision a un pas (prochain trimestre, pour les 16 branches) --------
# Covid=0 pour la prevision (hors periode de choc)
construire_X_prevision <- function(Y, p) {
  Tn <- nrow(Y); n <- ncol(Y)
  x <- c(1, 0, as.vector(t(Y[Tn:(Tn-p+1), , drop = FALSE])))
  matrix(x, nrow = 1)
}
