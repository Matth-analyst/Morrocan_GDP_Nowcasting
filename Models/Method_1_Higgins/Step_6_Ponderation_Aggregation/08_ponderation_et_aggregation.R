...
# Extension a 2010-2013 par repli sur les parts du premier trimestre disponible
# (2014T1) -- approximation documentee, faute de table de correspondance
# officielle entre les nomenclatures base 2007 (14 branches) et base 2014
# (16 branches) pour reconstruire les vraies parts nominales de cette periode.
premiere_part <- parts_reelles %>% filter(Date == DATE_DEBUT_BASE2014)
dates_manquantes <- seq(DATE_DEBUT_TRAIN, DATE_DEBUT_BASE2014 - months(3), by="3 months")
extension <- premiere_part[rep(1, length(dates_manquantes)), ]
extension$Date <- dates_manquantes
...
cat(sprintf("Parts nominales : %d trimestres (%d approximes par repli 2014T1, %d reels)\n",
            nrow(parts_completes), length(dates_manquantes), nrow(parts_reelles %>% filter(Date<=DATE_FIN_TRAIN))))

# ==============================================================================
# PARTIE C -- Agregation finale : Delta-log(PIB_t) = sum_i SH_{t-1}^i * Combine_i,t
# ==============================================================================
cat("\n=== Agregation finale ===\n")

# Les parts a utiliser au trimestre t sont celles du trimestre t-1 (SH_{t-1}),
# conformement a la formule de Higgins (equation 7)
parts_decalees <- parts_completes
parts_decalees$Date <- parts_decalees$Date %m+% months(3)  # decale d'un trimestre en avant
...
# Reference de comparaison : Delta-log de la somme des 16 VA reelles (proxy,
# PAS le PIB officiel qui inclut aussi les impots nets de subventions)
...
write_csv(resultat_final %>% select(Date, PIB_agrege, Reference_Somme16 = Somme16_ld),
          "resultats/croissance_PIB_agregee.csv")

cat(sprintf("\n%d trimestres agreges avec succes\n", sum(!is.na(resultat_final$PIB_agrege))))
cat("\nApercu (5 derniers trimestres du train) :\n")
print(tail(resultat_final %>% select(Date, PIB_agrege, Somme16_ld), 5))

# --- Figure -----------------------------------------------------------------
df_fig <- resultat_final %>% select(Date, PIB_agrege, Reference = Somme16_ld) %>%
...
p <- ggplot(df_fig, aes(x=Date, y=valeur, color=serie)) + geom_line(linewidth=0.7) +
  labs(title="Croissance du PIB agregee vs somme des 16 VA reelles (reference)",
       x=NULL, y="Delta-log", color=NULL) + theme_minimal(base_size=11)
ggsave("figures/PIB_agrege_vs_reference.png", p, width=9, height=4.5, dpi=150)
