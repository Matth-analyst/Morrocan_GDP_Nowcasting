# -*- coding: utf-8 -*-
"""
construire_classeur_ancres.py -- Classeur final avec un 4e bloc ANCRE
(en or), extrait de son bloc d'origine, place juste apres la CIBLE.
"""
import pickle
import pandas as pd
...
FILL_ANCRE = PatternFill("solid", fgColor="BF9000")   # or/moutarde
...
# Definition des ancres : (branche, colonne_source, freq_ancre)
ANCRES = {
    'agriculture':        [{'colonne': None, 'nom_match': 'Moyenne des precipitations'}],
    'peche':               [{'colonne': None, 'nom_match': 'POISSON PELAGIQUE', 'categorie':'especes'}],
    'industrie_transfo':   [{'colonne': None, 'nom_match': 'Taux d’Utilisation des Capacités', 'categorie':'TUC'}],
    'ind_extraction':      [{'colonne': 28, 'nom_match': None}],
    'immobilier':          [{'colonne': None, 'nom_match': "Crédits à l’immobilier"}],
    ...
    'electricite':         [{'colonne': 100, 'nom_match': None}],
    'infocom':             [{'colonne': None, 'nom_match': 'Parc téléphonie mobile global'}],
}

def trouver_ancre(indicateurs, specs):
    trouves = []
    for spec in specs:
...
for nom_feuille, prefix in BRANCHES_STANDARD:
    ...
    ancres = trouver_ancre(indicateurs, ANCRES[prefix])
    ancres_noms = set(id(a) for a in ancres)
    reste = [i for i in indicateurs if id(i) not in ancres_noms]
    ...
    col = ecrire_bloc(ws, col, f"ANCRE ({len(ancres)})", FILL_ANCRE, construire_dict_series(ancres))
    ...
    ancres_choisies[nom_feuille] = [a['nom'] for a in ancres]
    print(f"{nom_feuille:28s} : ancre(s)={[a['nom'][:40] for a in ancres]}")
...
ancre_immo = [c for c in credit_series if 'immobilier' in c['nom'].lower()][:1]
credit_reste = [c for c in credit_series if c not in ancre_immo]
...
col = ecrire_bloc(ws, col, f"ANCRE ({len(ancre_immo)})", FILL_ANCRE, construire_dict_series(ancre_immo))
col = ecrire_bloc(ws, col, f"TRIMESTRIEL - IPAI ({len(ipai_indicateurs)})", FILL_T, construire_dict_series(ipai_indicateurs))
col = ecrire_bloc(ws, col, f"MENSUEL - CREDIT ({len(credit_reste)})", FILL_M, construire_dict_series(credit_reste))
ws.freeze_panes = "A4"
recap.append(('Immobilier', len(ancre_immo), len(ipai_indicateurs), len(credit_reste)))
ancres_choisies['Immobilier'] = [a['nom'] for a in ancre_immo]
print(f"{'Immobilier':28s} : ancre(s)={[a['nom'][:40] for a in ancre_immo]}")

# --- Sommaire ---
ws = wb.create_sheet('Sommaire', 0)
ws.sheet_view.showGridLines = False
ws.merge_cells("A1:F1")
ws["A1"] = "GDPNow-Maroc — 434 séries retenues, avec ancres identifiées (Phase 2)"
...
headers = ["Branche", "Ancre(s)", "Trimestriel (reste)", "Mensuel (reste)", "Total", "Feuille"]
for j, h in enumerate(headers):
    c = ws.cell(row=3, column=j+1, value=h)
    c.font = Font(bold=True, color="FFFFFF"); c.fill = FILL_T; c.border = BORDER
for col_l, w in zip("ABCDEF", [28,14,16,14,10,25]):
...
r2=r+2
ws.cell(row=r2,column=1,value="Légende : vert=cible | or=ANCRE (a priori, non corrigée FDR) | bleu=trimestriel restant | orange=mensuel restant")
...
wb.save('/mnt/user-data/outputs/GDPNow_Maroc_series_avec_ancres.xlsx')
print(f"\nTotal ancres: {tot_a}")

with open(f'{DOSSIER}/resultats/ancres_choisies.pkl','wb') as f:
    pickle.dump(ancres_choisies, f)
