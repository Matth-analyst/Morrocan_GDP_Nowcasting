# -*- coding: utf-8 -*-
"""12_extraction_filtre_electricite.py -- Phase 2, Electricite-gaz-eau, sans fraicheur."""
...
cible_retro = extraire_serie(17, 18)
print(f"Cible retropolee : {len(cible_retro)} obs")

COLS_CIBLE_VARIANTES = set(range(103, 122))

indicateurs = []
for dc, val_cols in blocs:
    if dc in (1,17): continue
    for vc in val_cols:
...
with open('/home/claude/phase2/data/electricite_brut.pkl','wb') as f:
...
def categorie_economique(nom, source, colonne):
    ...
    if 'prix du brent' in n or 'prix du butane' in n: return 'prix_matieres_premieres'
    if colonne in range(19,24): return 'IPI_energetique'
    if 'raffinerie' in s: return 'raffinerie_intrants'
    if colonne in (100,101): return 'production_consommation_agregee'
    if 'ventes onee' in s or 'clients' in n or 'distributeurs' in n: return 'ventes_electricite_clientele'
    if 'énergie appelée' in s or 'energie.csv' in s: return 'production_electricite_detail'
    if 'combustibles' in s: return 'consommation_combustibles'
    return None
...
with open('/home/claude/phase2/resultats/electricite_sans_fraicheur.pkl','wb') as f:
