# -*- coding: utf-8 -*-
"""11_extraction_filtre_transports.py -- Phase 2, Transports, sans fraicheur."""
...
cible_retro = extraire_serie(14, 15)
print(f"Cible retropolee : {len(cible_retro)} obs")

COLS_CIBLE_VARIANTES = set(range(26, 45))
COLS_HORS_PERIMETRE = {16, 17, 21, 22}  # telecom (SMS/voix), creation d'entreprises

indicateurs = []
for dc, val_cols in blocs:
    if dc in (1,14): continue
    for vc in val_cols:
...
with open('/home/claude/phase2/data/transports_brut.pkl','wb') as f:
...
def categorie_economique(nom, source, colonne):
    ...
    if colonne in (5,6,20): return 'trafic_portuaire'
    if colonne == 7: return 'trafic_aerien'
    if colonne in (23,24): return 'cabotage'
    return None
...
for nom,col,cat,code in rejets_qualite:
    print(f"  col{col:3d} [{cat:18s}] {nom[:45]:45s} -- {code}")
print("\nRetenus:")
for c in candidats_qualite:
    print(f"  col{c['colonne']:3d} [{c['categorie']:18s}] {c['nom'][:45]:45s} {c['freq']:10s}")

with open('/home/claude/phase2/resultats/transports_sans_fraicheur.pkl','wb') as f:
