# -*- coding: utf-8 -*-
"""07_extraction_filtre_finances.py -- Phase 2, Finances et assurances, sans fraicheur."""
...
BRANCHE = 'Finances et assurances'
...
cible_retro = extraire_serie(13, 14)
print(f"Cible retropolee : {len(cible_retro)} obs")

COLS_CIBLE_VARIANTES = set(range(60, 78))
COLS_MISASSIGNEES = {92, 93}  # Tomates/Pommes de terre -- agriculture, sans rapport
COLS_VENTILATION_MACRO_SECTEUR = set(range(37, 56))  # par terme / macro-secteur, pas specifique a Finance

indicateurs = []
for dc, val_cols in blocs:
    if dc in (1,13): continue
    for vc in val_cols:
        if vc in COLS_CIBLE_VARIANTES or vc in COLS_MISASSIGNEES or vc in COLS_VENTILATION_MACRO_SECTEUR:
            continue
...
with open('/home/claude/phase2/data/finances_brut.pkl','wb') as f:
...
def categorie_economique(nom, source, colonne):
    ...
    if 'crédit bancaire' in n or 'crédits à la consommation' in n: return 'credit_bancaire'
    if colonne in range(10,12) or colonne in range(17,23): return 'credit_ventilation_objet'
    if colonne in range(23,37): return 'credit_secteur_institutionnel'
    if 'dépôts à vue' in n or 'masse monétaire' in n or 'avoirs officiels' in n: return 'agregats_monetaires'
    if colonne in (78,79): return 'assurance_agrege'
    if colonne in range(82,100) and colonne not in (92,93): return 'assurance_detail'
    if 'chèques postaux' in n: return 'cheques_postaux'
    return None
...
for nom,col,cat,code in rejets_qualite:
    print(f"  col{col:3d} [{cat:28s}] {nom[:45]:45s} -- {code}")
print("\nRetenus:")
for c in candidats_qualite:
    print(f"  col{c['colonne']:3d} [{c['categorie']:28s}] {c['nom'][:45]:45s} {c['freq']:10s}")

with open('/home/claude/phase2/resultats/finances_sans_fraicheur.pkl','wb') as f:
