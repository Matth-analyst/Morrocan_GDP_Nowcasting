# -*- coding: utf-8 -*-
"""08_extraction_filtre_hebergement.py -- Phase 2, Hebergement-restauration, sans fraicheur."""
...
BRANCHE = 'Hébergement-restauration'
...
cible_retro = extraire_serie(16, 17)
print(f"Cible retropolee : {len(cible_retro)} obs")

COLS_CIBLE_VARIANTES = set(range(53, 66))
COLS_CUMULE_EXCLUES = {20, 21}   # series cumulees intra-annuelles, non comparables
COL_SUSPECTE = {34}              # "Particuliers et MRE" (table credit par branche) -- hors sujet, verifie

indicateurs = []
for dc, val_cols in blocs:
    if dc in (1,16): continue
    for vc in val_cols:
        if vc in COLS_CIBLE_VARIANTES or vc in COLS_CUMULE_EXCLUES or vc in COL_SUSPECTE:
            continue
...
with open('/home/claude/phase2/data/hebergement_brut.pkl','wb') as f:
...
def categorie_economique(nom, source, colonne):
    ...
    if "taux d'occupation" in s: return 'taux_occupation_ville'
    if 'recettes touristiques' in s or 'recette' in n: return 'recettes'
    if 'nuitée' in s or 'nuitée' in n: return 'nuitees'
    if 'arriv' in s or 'arriv' in n: return 'arrivees'
    return None
...
for nom,col,cat,code in rejets_qualite:
    print(f"  col{col:3d} [{cat:22s}] {nom[:40]:40s} -- {code}")
print("\nRetenus:")
for c in candidats_qualite:
    print(f"  col{c['colonne']:3d} [{c['categorie']:22s}] {c['nom'][:45]:45s} {c['freq']:10s}")

with open('/home/claude/phase2/resultats/hebergement_sans_fraicheur.pkl','wb') as f:
