# -*- coding: utf-8 -*-
"""10_extraction_filtre_commerce.py -- Phase 2, Commerce, sans fraicheur."""
...
cible_retro = extraire_serie(11, 12)
print(f"Cible retropolee : {len(cible_retro)} obs")

COLS_CIBLE_VARIANTES = set(range(46, 61))
# Exclues : phosphate/soufre/energie (domaine d'autres branches), IDE (flux d'investissement,
# pas d'activite commerciale au sens strict)
COLS_HORS_PERIMETRE = {15,16,17,18,19,20,21,22,23,36,39,40,41}

indicateurs = []
for dc, val_cols in blocs:
    if dc in (1,11): continue
    for vc in val_cols:
        if vc in COLS_CIBLE_VARIANTES or vc in COLS_HORS_PERIMETRE:
            continue
...
with open('/home/claude/phase2/data/commerce_brut.pkl','wb') as f:
...
def categorie_economique(nom, source, colonne):
    ...
    if colonne in (37,38): return 'trafic_portuaire'
    if colonne in (42,43,44,58,59): return 'commerce_exterieur_global'
    if colonne in range(24,36): return 'conjoncture_prix_produits_finis'
    return None
...
for nom,col,cat,code in rejets_qualite:
    print(f"  col{col:3d} [{cat:32s}] {nom[:40]:40s} -- {code}")
print("\nRetenus:")
for c in candidats_qualite:
    print(f"  col{c['colonne']:3d} [{c['categorie']:32s}] {c['nom'][:45]:45s} {c['freq']:10s}")

with open('/home/claude/phase2/resultats/commerce_sans_fraicheur.pkl','wb') as f:
