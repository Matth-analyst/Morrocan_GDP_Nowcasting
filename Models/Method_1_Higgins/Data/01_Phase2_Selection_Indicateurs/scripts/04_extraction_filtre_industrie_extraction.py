# -*- coding: utf-8 -*-
"""
05_extraction_filtre_ind_extraction.py -- Phase 2 : extraction, filtre
economique, filtre de qualite (SANS fraicheur) pour Industrie d'extraction.
"""
...
BRANCHE = "Industrie d'extraction"
...
cible_retro = extraire_serie(15, 16)
print(f"Cible retropolee : {len(cible_retro)} obs")

COLS_CIBLE_VARIANTES = set(range(40, max_c+1))
indicateurs = []
for dc, val_cols in blocs:
    if dc in (1,15): continue
    for vc in val_cols:
...
with open('/home/claude/phase2/data/ind_extraction_brut.pkl','wb') as f:
...
def categorie_economique(nom, source, colonne):
    ...
    if 'prix international' in n: return 'prix_international'
    if 'crédit bancaire' in n: return 'credit_bancaire_sectoriel'
    if colonne in (12,13): return 'credit_ventilation_sectoriel'
    if n.startswith('ipm') or 'production minière' in s: return 'IPM'
    if 'phosphate' in n or 'soufre' in n or 'engrais' in n: return 'production_export_physique'
    if colonne in range(34,39): return 'production_export_sectoriel_bdd'
    return None
...
for nom,col,cat,code in rejets_qualite:
    print(f"  col{col:3d} [{cat:30s}] {nom[:40]:40s} -- {code}")

for c in candidats_qualite:
    print(f"  col{c['colonne']:3d} [{c['categorie']:30s}] {c['nom'][:45]:45s} {c['freq']:10s} {c['debut']} a {c['fin']}")

with open('/home/claude/phase2/resultats/ind_extraction_sans_fraicheur.pkl','wb') as f:
