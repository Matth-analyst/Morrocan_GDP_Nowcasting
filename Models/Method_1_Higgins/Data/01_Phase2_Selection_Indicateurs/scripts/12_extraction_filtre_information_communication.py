# -*- coding: utf-8 -*-
"""13_extraction_filtre_info_com.py -- Phase 2, Information-communication, sans fraicheur."""
...
cible_retro = extraire_serie(22, 23)
print(f"Cible retropolee : {len(cible_retro)} obs")

# Bug corrige : ne pas sauter tout le bloc dc=22 (qui contient aussi les 12
# candidats internet, colonnes 24-35, partageant la meme colonne de dates que
# la cible) -- exclure uniquement la colonne de la cible elle-meme (23).
# De meme, la plage des variantes de la cible (bloc dc=37) s'arrete a 48,
# pas 56 -- les colonnes 49-55 sont de vrais indicateurs (agregats ANRT),
# pas des variantes du PIB.
COL_CIBLE_ELLE_MEME = {23}
COLS_CIBLE_VARIANTES = set(range(38, 49))

indicateurs = []
for dc, val_cols in blocs:
    if dc == 1: continue
    for vc in val_cols:
        if vc in COL_CIBLE_ELLE_MEME or vc in COLS_CIBLE_VARIANTES: continue
...
with open('/home/claude/phase2/data/infocom_brut.pkl','wb') as f:
    pickle.dump(dict(cible_retropolee=cible_retro, indicateurs=indicateurs), f)

def categorie_economique(nom, source, colonne):
    n = nom.lower()
    if 'téléphonie mobile' in n or 'téléphonie fixe' in n or 'arpm' in n or 'trafic voix' in n:
        return 'telephonie'
    if colonne in range(24,36): return 'internet_fixe'
    if colonne in range(49,56): return 'agregats_sectoriels'
    return None
...
for nom,col,cat,code in rejets_qualite:
    print(f"  col{col:3d} [{cat:20s}] {nom[:45]:45s} -- {code}")

with open('/home/claude/phase2/resultats/infocom_sans_fraicheur.pkl','wb') as f:
