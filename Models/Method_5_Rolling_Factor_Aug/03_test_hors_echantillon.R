...
#
# Methode 3 -- Test hors-echantillon, meme fenetre que les Methodes 1 et 2
# (2021T2-2026T1, 20 trimestres), pour une comparaison directe des 3
# methodes.
# ==============================================================================
...
resultat <- read_csv("resultats/PIB_agrege_FactorAR.csv", show_col_types = FALSE)
...
cat(sprintf("RESULTAT DU TEST HORS-ECHANTILLON (Methode 3, Factor-Augmented AR) :\n"))
cat(sprintf("  n = %d trimestres\n", nrow(resultat_test)))
cat(sprintf("  Correlation = %.3f\n", corr_test))
cat(sprintf("  RMSFE (croissance annualisee) = %.2f points\n", rmsfe))

write_csv(data.frame(n=nrow(resultat_test), correlation=corr_test, RMSFE=rmsfe), "resultats/metriques_test_FactorAR.csv")
...
p <- ggplot(df_fig, aes(x=Date,y=valeur,color=serie)) + geom_line(linewidth=0.8) + geom_point(size=1.5) +
  labs(title="Methode 3 (Factor-Augmented AR) -- Test hors-echantillon (2021T2-2026T1)",
...
ggsave("figures/test_hors_echantillon_FactorAR.png", p, width=9, height=4.5, dpi=150)
cat("\nTermine.\n")
