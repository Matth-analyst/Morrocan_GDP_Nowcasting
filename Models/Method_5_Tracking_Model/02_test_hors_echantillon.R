...
resultat <- read_csv("resultats/PIB_agrege_Tracking.csv", show_col_types = FALSE)
...
cat(sprintf("RESULTAT DU TEST HORS-ECHANTILLON (Methode 5, Tracking Model) :\n"))
cat(sprintf("  n = %d trimestres\n", nrow(resultat_test)))
cat(sprintf("  Correlation = %.3f\n", corr_test))
cat(sprintf("  RMSFE (croissance annualisee) = %.2f points\n", rmsfe))

write_csv(data.frame(n=nrow(resultat_test), correlation=corr_test, RMSFE=rmsfe), "resultats/metriques_test_Tracking.csv")
...
p <- ggplot(df_fig, aes(x=Date,y=valeur,color=serie)) + geom_line(linewidth=0.8) + geom_point(size=1.5) +
  labs(title="Methode 5 (Tracking Model) -- Test hors-echantillon (2021T2-2026T1)",
...
ggsave("figures/test_hors_echantillon_Tracking.png", p, width=9, height=4.5, dpi=150)
cat("\nTermine.\n")
